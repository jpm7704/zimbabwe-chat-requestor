...
// Client-side representation of a field visit with camelCase properties
...
export function mapFieldVisitToFieldWorkRequest(visit: FieldVisit, requestTitle?: string, requestTicketNumber?: string): FieldWorkRequest {
  return {
    id: visit.id,
    ticketNumber: requestTicketNumber || 'N/A',
    title: requestTitle || visit.purpose || 'Field Visit',
    status: visit.status,
    ...
    reportSubmitted: visit.report_submitted || false,
    requestId: visit.request_id || null,
    purpose: visit.purpose || '',
    reportId: visit.report_id || null
  };
}
...
export function useFieldWork() {
      ...
      if (fetchError) {
        ...
        if (!data || data.length === 0) {
          ...
          const transformedVisits: FieldWorkRequest[] = data.map(visit =>
            mapFieldVisitToFieldWorkRequest(
              visit,
              visit.request?.title,
              visit.request?.ticket_number
            )
          );
  ...
  const createFieldVisit = async (visitData: Omit<FieldWorkRequest, 'id' | 'ticketNumber' | 'assignee' | 'reportSubmitted'>) => {
    try {
      ...
      const dbVisitData: Partial<FieldVisit> = {
